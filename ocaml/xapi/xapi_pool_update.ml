...
let get_attached_host ~__context ~self = 
...
let get_candidate_host ~__context ~self = 
  let attached_host = get_attached_host ~__context ~self in
...
let detach_helper ~__context ~uuid ~vdi =
  ...
  ( fun () ->
    ...
    if count = 1 then begin
      ...
      debug "pool_update.detach %s from %s" uuid mount_point;
...
let attach_helper ~__context ~uuid ~vdi =
  ...
  ( fun () ->
    ...
    if count = 0 then begin
      debug "pool_update.attach %s to %s" uuid mount_point;
...
let clean ~__context ~self ~host =
  ...
  if (get_candidate_host ~__context ~self) = host then begin
...
let pool_clean ~__context ~self =
  ...
  let host = get_candidate_host ~__context ~self in
  ignore(Helpers.call_api_functions ~__context
